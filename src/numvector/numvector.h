...
//#define r _Elems
	...
	public:
		...
		auto operator& (const numvector<double, 3>& y) const -> double
		{
			return r[0] * y[0] + r[1] * y[1] + r[2] * y[2];
		}
		...
		auto operator& (const numvector<P, n>& y) const -> typename std::remove_const<decltype(r[0] * y[0])>::type
		{
			typename std::remove_const<decltype(r[0] * y[0])>::type res = r[0]*y[0] + r[1]*y[1] + r[2]*y[2];			
			return res;
		}//operator&(...)
		*/
		...
		T normalize(P newlen = 1.0)
		{
			...
			auto ilen = static_cast<decltype(r[0] * newlen)>(newlen / oldlen);
			*this *= ilen;
			return oldlen;
		}//normalize(...)
